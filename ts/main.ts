...
class Partition {
  ...
  constructor(public readonly offset: number, public readonly filename:string) {}
  ...
  private async binChecksum() {
    ...
    const hash = await sha256(b.slice(0, b.length - 32));
...
class ESPLoader {
  ...
  constructor(private readonly port: PortController, private readonly image: ESPImage) {
  }
...
class Controller implements StreamTarget {
