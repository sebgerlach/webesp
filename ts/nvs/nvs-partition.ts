...
export class NVSPartition implements Partition {
  ...
  constructor(public readonly offset: number, public readonly filename: string, public size: number = 0x3000) {
    this.newPage();
  }
  ...
  public async load(): Promise<boolean> {
    ...
    for(const input of NVSPartition.entries) {
      const namespace = input.getAttribute('namespace') || NVSSettings.DEFAULT_NAMESPACE;
      const key = input.id;
